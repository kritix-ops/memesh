...
// other two surfaces (PosApp, CustomerApp) are NOT reachable. The
// RefundEntryModal that admin reuses from pos/ is legitimately copied here,
// so the surface-level check pins on the *App* filenames, not the directory.
...
test('apps/admin transitive imports never reach PosApp', async () => {
...
test('apps/admin transitive imports never reach CustomerApp', async () => {
  const graph = await collectImports(resolve(HERE, 'App.tsx'));
  for (const file of graph) {
    assert.ok(
...
test('positive control: walker actually reaches AdminApp.tsx', async () => {
  const graph = await collectImports(resolve(HERE, 'App.tsx'));
  const reached = [...graph].some((f) => f.endsWith('AdminApp.tsx'));
  assert.ok(reached, 'walker did not reach AdminApp.tsx — the negative assertions are vacuous');
});
